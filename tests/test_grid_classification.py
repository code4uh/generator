...
from circuit_array_spec.grid_classification import (
...
def test_iter_grid_coordinates_covers_complete_shape() -> None:
    coords = list(iter_grid_coordinates(width=2, height=2, layers=2))
    assert len(coords) == 8
...
def test_generated_grid_classification_accepts_complete_grid() -> None:
    classification = create_uniform_classification(width=3, height=2, layers=1, kind="wire")
...
def test_generated_grid_classification_rejects_missing_coordinate() -> None:
    ...
    with pytest.raises(ValueError, match="incomplete"):
        GeneratedGridClassification(width=2, height=1, layers=1, tiles=tiles)
...
def test_generated_grid_classification_rejects_invalid_kind() -> None:
    ...
    with pytest.raises(ValueError, match="invalid tile kinds"):
        GeneratedGridClassification(width=1, height=1, layers=1, tiles=tiles)
